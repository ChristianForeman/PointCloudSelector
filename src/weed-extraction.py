...
def rviz_arrow(pub, start, direction, name, thickness, length_scale, color):
    """
...
def main():
    ...
    src_pub = rospy.Publisher("source_weed", PointCloud2, queue_size=10)
    inliers_pub = rospy.Publisher("inliers_weed", PointCloud2, queue_size=10)
    arrow_pub = rospy.Publisher("normal", Marker, queue_size=10)
    ...
    while True:
        ...
        plot_pointcloud_rviz(src_pub,
                             pcd_points[:, 0],
                             pcd_points[:, 1],
                             pcd_points[:, 2])
        # Visualize filtered green points as "inliers"
        plot_pointcloud_rviz(inliers_pub,
                             green_pcd_points[:, 0],
                             green_pcd_points[:, 1],
                             green_pcd_points[:, 2])
        # Call rviz_arrow function to see normal of the plane
        rviz_arrow(arrow_pub, inlier_dirt_centroid, normal, name='normal', thickness=0.008, length_scale=0.15,
                   color='w')
        # Call plot_plane function to visualize plane in Rviz
        plot_plane(plane_pub, inlier_dirt_centroid, normal, size=0.1, res=0.001)
        rospy.sleep(1)
